...
> Answer: 
Null hypothesis: Difference bt sample mean and value is zero
Alternative: Difference bt sample mean and value is < 50 


```{r}
#general workflow:
#1 scatterplot - view the data
#2 create linear model - bestfit , assumes normal residuals, assumes causation
#3 examine model summary & plots

summary(EPAair$Ozone)
EPAair.subsample <- sample_n(EPAair, 5000)
#shapiro test doesn't like to run more than 5000 samples so we reduce it
...
Box.plot <- ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
  geom_boxplot()+
...
Violin.plot <- ggplot(EPAair, aes(y = Ozone, x = as.factor(Year))) +
...
PM2.5byOzone.reg <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(PM2.5byOzone.reg)
# p-value = 2.2E-16
# r-squared = 0.1314 #badnewsbears
...
the_cor <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% na.omit())

corrplot(the_cor, method ="ellipse")
corrplot.mixed(the_cor, upper = "ellipse")
...
the_lm <- lm(data = EPAair, PM2.5 ~ Ozone +
...
the_lm2 <- lm(data = EPAair, PM2.5 ~ Ozone +
                Month + SITE_LATITUDE + SITE_LONGITUDE)
summary(the_lm2)
```
