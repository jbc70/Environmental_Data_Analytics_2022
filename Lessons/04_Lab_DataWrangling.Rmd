...
#need the where clause to filter, so need to know class of data

# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)
#since depth is numeric, we can use an ==
#for character we can use %in% to specify vectors
...
#can use %in% clause with numbers too using a range notation, inclusive
...
NTL.phys.data.TuesdayLake1990thru1999 <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake") & year4 %in% c(1990:1999))
dim(NTL.phys.data.TuesdayLake1990thru1999)
#alternatively:
#NTL.phys.data.TuesdayLake1990thru1999 <- 
 # NTL.phys.data %>%
...
Excercise4 <- Excercise3 %>%
  filter(depth == 0) %>%
  filter(drop_na(temperature_C)) %>%
  summarise()
