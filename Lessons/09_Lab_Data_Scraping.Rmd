...
#structure of a URL:
  #https is secure protocol
  #web address is internet location
  #uv is name of the service, then ?, then the parameters for the service
  #each parameter is separated by &
  #rdb = relational data base, or tab-delimited format

#Get the data, which starts on line 30
gage_data <- read.table(theURL,skip = 28,header=TRUE,sep='\t')
#skip = 29 means skip those first 28 lines of comments
...
#Get the data, which starts on line 30
gage_data2 <- read.table(theURL2,skip = 29,header=TRUE,sep='\t')
#skip = 29 means skip those first 29 lines of comments
...
> "Your key goes here" is to ask for a password of sorts, have to sign up for a key from the census org to get a key
> Always read the terms of service, may specify what you can or can't do with it, such as no downloads, scrapes, or shares
> Can also look for a robots.txt file by adding "robots.txt" in the URL. Robots.txt stops web crawlers like google from just pulling everything, and disallow will list whatever parts you cannot scrape.
...
#the above is a tidycensus command to install the census key after requesting one from the census bureau - need to request one other than John's
...
scrape.it <- function(the_year){
  #Get the proper url
  the_url <- ifelse(the_year==2020, #current year
                    'https://www.eia.gov/electricity/state/',
