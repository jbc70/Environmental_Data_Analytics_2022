...
neusehuc8_sf <- st_read("./Data/Spatial/NCHUC8.shp") %>%
  filter(SUBBASIN == "Upper Neuse")

colnames(neusehuc8_sf)

#Create a map
mapview(neusehuc8_sf, col.regions = "purple") + 
  mapview(nchuc8_sf, col.regions = "orange")
#OR 
ggplot() +
  geom_sf(data = nchuc8_sf, fill = "orange") +
  geom_sf(data = neusehuc8_sf, fill = "purple")
...
huc8_wind_raw <- 
  read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c("HUC_12" = "factor")) %>%
...
nc_wind_huc8 <- nchuc8_sf %>%
  left_join(huc8_wind_raw, by = c("HUC_8" = "HUC_8"))

colnames(nc_wind_huc8)

#View the outputs
mapview(nc_wind_huc8, zcol = "SumWindEnergy")
