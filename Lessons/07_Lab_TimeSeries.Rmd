...
#separate function does something similar to mutate, it breaks one column into multiple
#then mutate to create date object w/ paste function to do the opposite of separate
#lubridate function 'my' is a date function
...
#need to convert to time series object because the functions for time series analysis can only read time series objects, not dataframes
#still need dataframe because ggplot doesnt like time series

#lubridate is handy to find first of each month and year
firstmonth <- month(first(BRA_complete$Date))
firstyear <- year(first(BRA_complete$Date))

BRA_rainfall.ts <- ts(BRA_complete$Rainfall, start = c(firstyear,firstmonth), frequency = 12)
BRA_temperature.ts <- ts(BRA_complete$Temperature_C, start = c(firstyear,firstmonth), frequency = 12)
#year comes first in the c() section of this function
```
...
rainfall_decomp <- stl(BRA_rainfall.ts, s.window = "periodic")
temp_decomp <- stl(BRA_temperature.ts, s.window = "periodic")
...
ggplot(rainfall_components) +
  geom_line(aes(y = Observed, x = Date), size = 0.3) +
  geom_line(aes(y = trend, x = Date), color = "red") +
  geom_line(aes(y = seasonal, x = Date), color = "blue") +
  #geom_hline(yintercept = 0, lty = 2) +
  ylab ("Rainfall")

#do the same for temperature and set colors for each geom_line
